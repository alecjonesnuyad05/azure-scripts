...
#       [--dry-run]
#
# --dry-run: logs into both sides, reads the source disk's size/sku/location,
# checks whether the destination disk already exists, and prints exactly
# what would happen — but creates no snapshot, no SAS, no staging account/
# container, runs no azcopy, and creates/deletes no destination disk.
#
...
DROP_EXISTING=0 ; KEEP_STAGING=0 ; DRY_RUN=0

die()  { echo "ERROR: $*" >&2; exit 1; }
info() { echo ">>> $*" >&2; }

usage() { sed -n '2,61p' "$0" | sed 's/^# \{0,1\}//'; exit "${1:-0}"; }
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  if az disk show -g "$DST_RG" -n "$DST_DISK" >/dev/null 2>&1; then
    if [[ "$DROP_EXISTING" -eq 1 ]]; then
      info "[dry-run] Would delete existing destination disk '$DST_DISK' (--drop-existing)."
    else
      die "destination disk '$DST_DISK' already exists (use --drop-existing to replace)"
    fi
  fi
  info "[dry-run] Would snapshot source disk '$DISK' as '$SNAPSHOT_NAME', grant a ${SAS_DURATION}s read-only SAS on it,"
  info "[dry-run] stage it via azcopy through storage account '${STAGING_ACCOUNT:-<auto-generated>}' / container '$STAGING_CONTAINER',"
  info "[dry-run] then create destination disk '$DST_DISK' (~${SRC_SIZE_GB}GB, sku=$SKU) in '$DST_RG' ($DST_LOCATION) — all skipped."
